...
const SeoFadingWords: React.FC = () => {
  ...
  const createParticle = (): Particle => {
    const margin = 120; // margin to avoid clipping at edges
    return {
      x: randomRange(margin, window.innerWidth - margin),
      y: randomRange(margin, window.innerHeight - margin),
      ...
      particlesRef.current.forEach((p) => {
        ...
        if (p.fadingIn) {
          ...
          if (p.opacity <= 0) {
            ...
            // Reset particle data
            const margin = 120;
            p.x = randomRange(margin, window.innerWidth - margin);
            p.y = randomRange(margin, window.innerHeight - margin);
